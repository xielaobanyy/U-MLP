...
def main():
    ...
    if config['optimizer'] == 'Adam':
        ...
        optimizer = optim.AdamW(params, lr=config['lr'], betas=(0.9, 0.99), weight_decay=config['weight_decay'])
    else:
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset,
        # CutMix(train_dataset,2,beta=1.0,prob=0.5,num_mix=2),
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset,
        # CutMix(val_dataset, 2, beta=1.0, prob=0.5, num_mix=2),
        batch_size=config['batch_size'],
        shuffle=False,
        num_workers=config['num_workers'],
        drop_last=False)
    ...
    for epoch in range(config['epochs']):
        ...
        if epoch % 20 == 0:
            ...
            res["train_loss"] = res["loss"]
            res["train_iou"] = res["iou"]
            ...
            plt.subplot(1, 2, 2)
            plt.title(title[1])
            plt.plot(res["train_iou"])
            plt.plot(res["val_iou"])
            plt.xlabel("epoch")
            plt.legend(labels=['train_iou', "val_iou"])
            plt.grid()

            plt.savefig('models/%s/accuracy.png' % config['name'])
            plt.close()
        trigger += 1
